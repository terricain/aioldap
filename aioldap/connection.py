...
from aioldap.exceptions import LDAPBindException, LDAPStartTlsException, LDAPChangeException, LDAPModifyException, \
    LDAPDeleteException, LDAPAddException, LDAPExtendedException, OperationNotSupported
        ...
        if len(data) > 0:
            ...
            while self.messages:
                ...
                if msg_asn['protocolOp'] == 1:  # Bind request, only 1, finished after this
                    ...
                    logger.debug('data_received: id {0}, refs response'.format(msg_id))
...
class LDAPConnection(object):
    ...
    async def bind(self, bind_dn: str = None, bind_pw: str = None):
        ...
        if self._proto is None or self._proto.transport.is_closing():
            self._socket, self._proto = await self.loop.create_connection(lambda: LDAPClientProtocol(self.loop),
    ...
    async def search(self, search_base: str, search_filter: str, search_scope: str = 'SUBTREE',
                     dereference_aliases: str = 'ALWAYS',
                     attributes: Optional[Union[str, List[str]]] = None, size_limit: int = 0, time_limit: int = 0,
                     types_only: bool = False,
                     auto_escape: bool = True, auto_encode: bool = True, schema=None, validator=None,
                     check_names: bool = False, cookie=None,
    ...
    async def paged_search(self, search_base: str, search_filter: str, search_scope: str = 'SUBTREE',
                           dereference_aliases: str = 'ALWAYS',
                           attributes: Optional[Union[str, List[str]]] = None, size_limit: int = 0, time_limit: int = 0,
                           types_only: bool = False,
                           auto_escape: bool = True, auto_encode: bool = True, schema=None, validator=None,
        ...
        while cookie is not None and cookie != b'':
            response = await self.search(
                ...
                auto_escape=auto_escape, auto_encode=auto_encode, schema=schema, validator=validator,
                check_names=check_names,
                timeout=timeout, get_operational_attributes=get_operational_attributes, page_size=page_size,
